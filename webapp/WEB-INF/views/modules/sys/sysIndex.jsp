...
<head>
	...
	<style type="text/css">
		...
		#header {margin:0 0 8px;position:static;} #header li {font-family:Microsoft YaHei;font-size:14px;_font-size:12px;}
	...
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner">
				...
				<div class="nav-collapse">
					<ul id="menu" class="nav" style="*white-space:nowrap;float:none;font-family: Microsoft YaHei;">
						<c:set var="firstMenu" value="true"/>
						<c:forEach items="${fns:getMenuList()}" var="menu" varStatus="idxStatus">
							<c:if test="${menu.parent.id eq '1'&&menu.isShow eq '1'}">
								<li class="menu ${not empty firstMenu && firstMenu ? ' active' : ''}">
									<c:if test="${empty menu.href}">
										<a class="menu" style="font-family: Microsoft YaHei;" href="javascript:" data-href="${ctx}/sys/menu/tree?parentId=${menu.id}" data-id="${menu.id}"><span>${menu.name}</span></a>
									</c:if>
									<c:if test="${not empty menu.href}">
										<a class="menu" style="font-family: Microsoft YaHei;" href="${fn:indexOf(menu.href, '://') eq -1 ? ctx : ''}${menu.href}" data-id="${menu.id}" target="mainFrame"><span>${menu.name}</span></a>
									</c:if>
								</li>
								<c:if test="${firstMenu}">
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	            Copyright &copy; 2015-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="${fns:getConfig('productAddress')}" target="_blank">${fns:getConfig('autor')}</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
