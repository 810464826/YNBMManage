...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/member/member/">会员管理列表</a></li>
		<li class="active"><a href="${ctx}/member/member/form?id=${member.id}">会员管理<shiro:hasPermission name="member:member:edit">${not empty member.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="member:member:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="member" action="${ctx}/member/member/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">升级订单号：</label>
			<div class="controls">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:hidden id="faceicon" path="faceicon" htmlEscape="false" maxlength="1024" class="input-xlarge"/>
				<sys:ckfinder input="faceicon" type="files" uploadPath="/member/member" selectMultiple="true"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input name="payTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${member.payTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
